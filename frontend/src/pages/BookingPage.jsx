...
export default function BookingPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      await addBookingGuests(activeBooking.id, guests.map(({ key, ...guest }) => ({ ...guest, age: Number(guest.age) })))
      setMessage(`Booking #${activeBooking.id} is confirmed for €${activeBooking.amount}.`)
      setStatus('success')
    } catch (error) {
      setMessage(activeBooking
        ? `${error.message}. Booking #${activeBooking.id} remains held; correct the guest details and retry without reserving twice.`
        : error.message)
      setStatus('error')
    }
  }
