  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (activeBooking.bookingStatus === 'RESERVED') {
        activeBooking = await addBookingGuests(activeBooking.id, guests.map(({ key, ...guest }) => ({ ...guest, age: Number(guest.age) })))
        setBooking(activeBooking)
      }
      activeBooking = await confirmBooking(activeBooking.id)
      setBooking(activeBooking)
